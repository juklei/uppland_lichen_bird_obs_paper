...
model <- "scripts/JAGS/bird_JAGS_bpo_cov.R"
...
capture.output(summary(zc), HPDinterval(zc, prob = 0.95)) %>% 
  write(., "results/bpo/parameters_bird_cov_2018.txt")

## 5. Validate the model and export validation data and figures ----------------

pdf("figures/plot_zc_bird_cov_2018.pdf")
plot(zc)
dev.off()

capture.output(raftery.diag(zc), heidel.diag(zc)) %>% 
  write(., "results/bpo/diagnostics_bird_cov_2018.txt")
...
zj_pred <- jags.samples(jm, 
                        variable.names = c("rho", "richness", "p_det"),
                        n.iter = samples, 
                        thin = n.thin)
